...
function style(){
  ...
  const s=document.createElement('style');s.id='ownerDataStyle';
  s.textContent=`
    ...
    .ownerLogged{padding:12px;border-radius:14px;background:#e8f8f0;color:#16805a;margin-top:12px;text-align:center;font-weight:800;font-size:13px}
    .ownerLogout{width:100%;padding:11px;border-radius:13px;background:#ffe7eb;color:#d83d54;font-weight:800;margin-top:10px}
  `;document.head.appendChild(s);
}
...
async function boot(){
  ...
  addMenu();
  const menu=document.getElementById('menu');if(menu)new MutationObserver(()=>setTimeout(addMenu,0)).observe(menu,{childList:true,subtree:true});
  if(ok){setTimeout(()=>openOwnerData(),250)}
}

window.openOwnerData=openOwnerData;window.closeOwnerData=closeOwnerData;window.ownerGoogleLogin=ownerGoogleLogin;window.ownerLogout=ownerLogout;
